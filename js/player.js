const Player = (() => {
  ...
  const FACTORY_DEFAULTS = {speed:0.07, fontSize:64, font:'Verdana', fontColor:'#ffffff', bgColor:'#000000', position:'random', autoFit:true, randomFontColor:true, order:'sequential', shuffleImages:true, imageScale:40, imageOpacity:100, imageRotation:0, imagePosition:'same', imageAnimation:'none', randomImageSize:false, randomImageMin:40, randomImageMax:70};
  function init(){
  ...
  function resetFactoryDefaults(){ applySettings(FACTORY_DEFAULTS); if(running) showCurrent(false); Storage.saveEverything(); }
  function escapeHtml(s){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
  return {init,start,stop,getSettings,applySettings,resetFactoryDefaults,fullscreen,next,prev};
})();
